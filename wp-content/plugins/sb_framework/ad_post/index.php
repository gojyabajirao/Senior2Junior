...
function sb_send_mails_on_publish($new_status, $old_status, $post) {
    if ('publish' !== $new_status or 'publish' === $old_status or 'ad_post' !== $post->post_type) {
        return;
    }
    global $sitepress;
    global $adforest_theme;
    if (isset($adforest_theme['email_on_ad_approval']) && $adforest_theme['email_on_ad_approval']) {
        //do_action('adforest_check_post_duplication', $post);
        if ((function_exists('icl_object_id') || class_exists('SitePress'))) {
            ...
            if ($_icl_lang_duplicate_of) {
                ...
                adforest_get_notify_on_ad_approval($post->ID);
            }
        } else {
